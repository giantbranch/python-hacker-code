...
class LIB:
    ...
    def related_libs(self, names):
        if not isinstance(names, list):
            names = [names]
        
        for name in names:
            if name not in self.__related_libs__.keys():
                lib = LIB(name)
                self.__related_libs__[name]=lib
                self.symbols.update(lib.symbols)
    
    def dump(self):
        print "LIB DUMP"
        self.real_dump(self)
        for lib in self.__related_libs__.values():
            self.real_dump(lib)
        
    def real_dump(self, lib):
        print "Linker Offsets"
        linker_member = lib.SecondLinkerMember
        for name,index in linker_member.Symbols:
            # find the obj which contains our symbol:
            member_offset = linker_member.MemberOffsets[index-1]
            print "%s: %x"%(name,member_offset)
        
        print "Members"
        for member_offset,objs in lib.Objects.iteritems():
            c = 0
            obj = objs.Object
            if hasattr(obj, "SymbolTableByName"):
                #normal COFF
                print "COFF member=%x"%member_offset
                c+=4
                print " "*c+"Symbols"
                c+=4
                a=0
                for symbol in obj.SymbolTable:
                    if symbol == None:
                        print " "*c+"num=%d, auxiliary symbol"%a
                    else:
                        print " "*c+"num=%d, name=%s, section=%d, value=%x, type=%d, storage=%d"%(a,symbol.Name, symbol.SectionNumber, symbol.Value, symbol.Type, symbol.StorageClass)
                    a+=1
                c-=4
                print " "*c+"Sections"
                c+=4
                ...
                for section in obj.sections:
                    print " "*c+"number=%d, name=%s, nrelocs=%d, sizeraw=0x%x, VA=0x%x"%(a,section.Name.strip("\x00"),section.NumberOfRelocations,section.SizeOfRawData,section.VirtualAddress)
                    #XXX:debug
                    if "\x64\xa1\x00\x00\x00\x00\x50\x64\x89\x25\x00\x00\x00\x00\x83\xc4\xf0\x53\x56\x57\x89\x65\xe8\xff\x15" in section.get_data(0):
                        print "THIS IS AN ENTRY POINT ADDRESS"
                    if a==4 and ".data" in section.Name and section.NumberOfRelocations==2 and section.SizeOfRawData==0x280:
                        hexdump(section.__parent__.__data__[section.PointerToRawData:section.PointerToRawData+section.SizeOfRawData])
                    a+=1
                    print " "*(c+4)+"Relocations"
                    for reloc in section.relocations:
                        print " "*(c+8)+"VA:%x, Symbol:%d, Type:%d"%(reloc.VirtualAddress,reloc.SymbolTableIndex, reloc.Type)
            else:
                #short import
                print " "*c+"short import. member=%x, importname:%s, dllname:%s"%(member_offset, obj.ImportName, obj.DLLName)
            
        
        
    def link(self, names, base=0x00400000, plt_size=6):
        """
        receive a list of functions to link, a base address to start and optionally
          size of a JMP DWORD PTR DS:[xxxx] instruction (for PLT)
        return linear linked code (with enough space for the PLT before the code)
        """
        ...
        self.link_idx=0
        self.func_num=0
        self.jmp_table_size=0
        self.link_sections={}
        self.link_relocations=[]
        self.link_imports_by_num={}
        ...
        #collect dependecies
        for name in names:
            print "START link(%s, %08x)"%(name,base)
            self.link_dependencies(name)
        
        real_binary = ""
        idx = 0
        
        #first reserve space for the imports
        real_binary += "\x90"*(self.func_num*plt_size)
        idx += self.func_num*plt_size
        self.link_imports_by_num = dict([(w[2],w) for q,w in self.link_imports.iteritems()])
        import_nums=self.link_imports_by_num.keys()
        import_nums.sort()
        for num in import_nums:
            i = self.link_imports_by_num[num]
            print "import: %s %s (%d)"%(i[0], i[1], i[2])
            real_binary += struct.pack("<L", base + (num*plt_size))
            idx += 4

        self.jmp_table_size=idx
        #handle relocations
        for reloc in self.link_relocations:
            ra = reloc[0]
            rel_addr = reloc[1]
            type = reloc[2]
            flags = reloc[3]
            if   flags=="import":
                #at beginning there's all the jumps and pointers
                rel_addr = self.func_num*plt_size + rel_addr*4
            elif flags=="absolute":
                #IMAGE_SYM_ABSOLUTE values are always relocated as IMAGE_REL_I386_DIR32
                #we'll add base later, so we end up with the original value
                rel_addr -= base
            else:
                rel_addr += self.jmp_table_size
            
            if   I386_COFF_RELOCATION_TYPES[type] == "IMAGE_REL_I386_DIR32":
                rel_addr += base
            elif I386_COFF_RELOCATION_TYPES[type] == "IMAGE_REL_I386_DIR32NB":
                pass
            elif I386_COFF_RELOCATION_TYPES[type] == "IMAGE_REL_I386_REL32":
                rel_addr -= ra + self.jmp_table_size + 4
            else:
                print "relocation type not supported: %s"%I386_COFF_RELOCATION_TYPES[type]
                raise Exception
            
            self.link_binary = self.link_binary[:ra] + struct.pack("<L", rel_addr) + self.link_binary[ra+4:]
        
        return real_binary+self.link_binary
        
    def link_dependencies(self, name, symbol=None):
        if symbol == None:
            try:
                symbol=self.symbols[name]
            except KeyError:
                print "symbol %s cannot be resolved"%name
                raise KeyError
        
        process_relocations = False
        offset = symbol.__parent__.LIBOffset
        if not self.link_symbols.has_key(offset):
            self.link_symbols[offset] = {}

        if hasattr(symbol, "ImportName"):
            #short import symbol
            impname = symbol.ImportName
            
            #we already got this import
            if self.link_imports.has_key(name):
                return symbol
            
            if   symbol.NameType == SHORT_IMPORT_NAME_TYPES['IMPORT_NAME_NOPREFIX']:
                while impname[0] == '?' or impname[0] == '@' or impname[0] == '_':
                    impname = impname[1:]
            elif symbol.NameType == SHORT_IMPORT_NAME_TYPES['IMPORT_NAME_UNDECORATE']:
                while impname[0] == '?' or impname[0] == '@' or impname[0] == '_':
                    impname = impname[1:]
                impname = impname.split('@')[0]
            self.link_imports[name] = ( impname, symbol.DLLName, self.func_num )
            self.func_num+=1
        else:
            #normal symbol
            sect_id="%x-%x"%(offset, symbol.SectionNumber-1)
            
            #we already got this symbol
            if self.link_symbols[offset].has_key(symbol.Name):
                return symbol
            
            if   symbol.StorageClass == IMAGE_SYMBOL_CLASSES['IMAGE_SYM_CLASS_EXTERNAL']:
                if   symbol.SectionNumber == IMAGE_SYM_UNDEFINED:
                    if   symbol.Value == 0:
                        #real external symbol, resolve agains public symbols
                        self.link_dependencies(symbol.Name)
                    else:
                        #uninitialized buffer
                        self.link_binary+="\x00"*symbol.Value
                        self.link_symbols[offset][symbol.Name] = self.link_idx
                        self.link_idx+=symbol.Value
                elif symbol.SectionNumber == IMAGE_SYM_ABSOLUTE:
                    #absolute DWORD
                    self.link_symbols[offset][symbol.Name] = ( symbol.Value, "absolute" )
                elif symbol.SectionNumber > 0:
                ...
                self.link_symbols[offset][symbol.Name] = sect_idx + symbol.Value
                    ...
                    if not self.link_sections.has_key(sect_id):
                        process_relocations = True
                        #may be we don't get the real section yet, do it now.
                        data = symbol.__section__.get_data(0)
                        self.link_binary+=data
                        sect_idx=self.link_idx
                        self.link_idx+=len(data)
